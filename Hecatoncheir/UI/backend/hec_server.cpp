#include <iostream>
#include <string>
#include <vector>
#include <unordered_map>
#include <mutex>
#include <nlohmann/json.hpp>
#include "../../API/Hecatoncheir.h" 
#include "../../include/def.h"
#include "../../include/utils.h"

using json = nlohmann::json;

std::vector<std::string> hosts;
std::string pointsPath;
std::string queriesPath;
int pointDatasetID = -1;
int secondPointDatasetID = -1;
int predicateValue;
int k;
std::string fileFormat;
std::string geometryType;
std::string queryFileFormat;
std::string queryGeometryType;
std::vector<hec::Query*> batch;
std::unordered_map<int, std::unique_ptr<hec::QResultBase>> results;
hec::DistanceJoinQuery* distanceQuery = nullptr;
hec::PredicateJoinQuery* predicateJoinQuery = nullptr;

bool hec_initialized = false;
bool data_prepared = false;
std::string current_query_type;
double start, total_time;
int ret;
std::mutex hec_mutex;

hec::QueryType mapPredicateToJoinType(const std::string& predicate) {
    static std::unordered_map<std::string, hec::QueryType> joinTypeMap = {
        {"intersect", hec::Q_INTERSECTION_JOIN},
        {"inside", hec::Q_INSIDE_JOIN},
        {"contains", hec::Q_CONTAINS_JOIN},
        {"covers", hec::Q_COVERS_JOIN},
        {"coveredBy", hec::Q_COVERED_BY_JOIN},
        {"equals", hec::Q_EQUAL_JOIN},
        {"meet", hec::Q_MEET_JOIN},
        {"findRelation", hec::Q_FIND_RELATION_JOIN},
        {"disjoint", hec::Q_DISJOINT_JOIN}
    };
    return joinTypeMap.count(predicate) ? joinTypeMap[predicate] : hec::Q_INTERSECTION_JOIN;
}

int main() {
    std::ios::sync_with_stdio(false);
    std::cout.setf(std::ios::unitbuf);

    std::string line;
    while (std::getline(std::cin, line)) {
        try {
            auto cmd = json::parse(line);
            std::string action = cmd["action"];
            json response;
            std::lock_guard<std::mutex> lock(hec_mutex);
            
            if (action == "init") {
                hosts = cmd["hosts"].get<std::vector<std::string>>();
                ret = hec::init(hosts.size(), hosts);
                if(ret != DBERR_OK){
                    logger::log_error(ret, "Error in init:");
                }
                hec_initialized = (ret == DBERR_OK);
                response["status"] = hec_initialized ? "success" : "error";
            } 
            else if (action == "prepare") {
                // Clean up previous data if any
                if (data_prepared) {
                    if (pointDatasetID >= 0) {
                        hec::unloadDataset(pointDatasetID);
                        pointDatasetID = -1;
                    }
                    if (secondPointDatasetID >= 0) {
                        hec::unloadDataset(secondPointDatasetID);
                        secondPointDatasetID = -1;
                    }
                    if (distanceQuery != nullptr) {
                        delete distanceQuery;
                        distanceQuery = nullptr;
                    }
                    if (predicateJoinQuery != nullptr) {
                        delete predicateJoinQuery;
                        predicateJoinQuery = nullptr;
                    }
                    for (auto& q : batch) {
                        auto it = results.find(q->getQueryID());
                        if (it != results.end()) {
                            
                        }
                        delete q;
                    }
                    batch.clear();
                    results.clear();
                    data_prepared = false;
                }

                if (!hec_initialized) {
                    response["status"] = "error";
                    response["message"] = "HEC not initialized";
                    std::cout << "C++: " << response.dump() << std::endl;
                    std::cout.flush();
                    continue;
                }

                current_query_type = cmd["queryType"];
                std::string dataset = cmd.value("dataset", "");
                std::string queryDataset = cmd.value("queryDataset", "");
                k = cmd.value("kValue", 5);
                
                // Extract format and geometry type from command
                fileFormat = cmd.value("fileFormat", "WKT");
                geometryType = cmd.value("geometryType", "POINT");
                queryFileFormat = cmd.value("queryFileFormat", "WKT");
                queryGeometryType = cmd.value("queryGeometryType", "POINT");

                if (current_query_type == "knnQuery") {
                    pointsPath = dataset;
                    pointDatasetID = hec::prepareDataset(pointsPath, fileFormat, geometryType, false);
                    if (pointDatasetID < 0) {
                        response["status"] = "error";
                        response["message"] = "Failed to load dataset";
                    } else {
                        start = hec::time::getTime();
                        ret = hec::partition({pointDatasetID});
                        ret = hec::buildIndex({pointDatasetID}, hec::IT_UNIFORM_GRID);
                        queriesPath = queryDataset;
                        batch = hec::loadKNNQueriesFromFile(queriesPath, queryFileFormat, pointDatasetID, k);
                        data_prepared = true;
                        response["status"] = "success";
                    }
                } 
                else if (current_query_type == "rangeQuery") {
                    pointsPath = dataset;
                    pointDatasetID = hec::prepareDataset(pointsPath, fileFormat, geometryType, false);
                    if (pointDatasetID < 0) {
                        response["status"] = "error";
                        response["message"] = "Failed to load dataset";
                    } else {
                        start = hec::time::getTime();
                        ret = hec::partition({pointDatasetID});
                        ret = hec::buildIndex({pointDatasetID}, hec::IT_UNIFORM_GRID);
                        queriesPath = queryDataset;
                        batch = hec::loadRangeQueriesFromFile(queriesPath, queryFileFormat, pointDatasetID, hec::QR_COUNT);
                        data_prepared = true;
                        response["status"] = "success";
                    }
                } 
                else if (current_query_type == "distanceJoins") {
                    double distance = cmd.value("distance", 0.001);
                    
                    std::cout << "DEBUG: Loading distance join datasets:" << std::endl;
                    std::cout << "  Dataset: " << dataset << std::endl;
                    std::cout << "  Query Dataset: " << queryDataset << std::endl;
                    std::cout << "  Distance: " << distance << std::endl;
                    
                    pointsPath = dataset;
                    pointDatasetID = hec::prepareDataset(pointsPath, fileFormat, "POINT", false);
                    secondPointDatasetID = hec::prepareDataset(queryDataset, queryFileFormat, "POINT", false);
                    
                    std::cout << "DEBUG: Dataset IDs: " << pointDatasetID << ", " << secondPointDatasetID << std::endl;
                    
                    if (pointDatasetID < 0 || secondPointDatasetID < 0) {
                        response["status"] = "error";
                        response["message"] = "Failed to load datasets";
                        std::cout << "ERROR: Dataset loading failed" << std::endl;
                    } else {
                        std::cout << "DEBUG: Partitioning datasets..." << std::endl;
                        ret = hec::partition({pointDatasetID, secondPointDatasetID});
                        if (ret != DBERR_OK) {
                            logger::log_error(ret, "Partition error:");
                            response["status"] = "error";
                            response["message"] = "Partitioning failed";
                        } else {
                            std::cout << "DEBUG: Building indexes..." << std::endl;
                            ret = hec::buildIndex({pointDatasetID, secondPointDatasetID}, hec::IT_UNIFORM_GRID);
                            if (ret != DBERR_OK) {
                                logger::log_error(ret, "Index build error:");
                                response["status"] = "error";
                                response["message"] = "Index building failed";
                            } else {
                                std::cout << "DEBUG: Creating distance query..." << std::endl;
                                distanceQuery = new hec::DistanceJoinQuery(pointDatasetID, secondPointDatasetID, 0, hec::QR_COLLECT, distance);
                                if (distanceQuery == nullptr) {
                                    response["status"] = "error";
                                    response["message"] = "Failed to create distance query";
                                    std::cout << "ERROR: Distance query creation failed" << std::endl;
                                } else {
                                    data_prepared = true;
                                    response["status"] = "success";
                                    std::cout << "DEBUG: Distance query prepared successfully" << std::endl;
                                }
                            }
                        }
                    }
                }
                else if (current_query_type == "spatialJoins") {
                    std::string predicate = cmd.value("predicate", "intersect");
                    
                    std::cout << "DEBUG: Loading spatial join datasets:" << std::endl;
                    std::cout << "  Dataset: " << dataset << std::endl;
                    std::cout << "  Query Dataset: " << queryDataset << std::endl;
                    std::cout << "  Predicate: " << predicate << std::endl;
                    std::cout << "  Geometry Type: " << geometryType << std::endl;
                    std::cout << "  Query Geometry Type: " << queryGeometryType << std::endl;
                    
                    pointsPath = dataset;
                    pointDatasetID = hec::prepareDataset(pointsPath, fileFormat, geometryType, false);
                    secondPointDatasetID = hec::prepareDataset(queryDataset, queryFileFormat, queryGeometryType, false);
                    
                    std::cout << "DEBUG: Dataset IDs: " << pointDatasetID << ", " << secondPointDatasetID << std::endl;
                    
                    if (pointDatasetID < 0 || secondPointDatasetID < 0) {
                        response["status"] = "error";
                        response["message"] = "Failed to load datasets";
                        std::cout << "ERROR: Dataset loading failed" << std::endl;
                    } else {
                        std::cout << "DEBUG: Partitioning datasets..." << std::endl;
                        ret = hec::partition({pointDatasetID, secondPointDatasetID});
                        if (ret != DBERR_OK) {
                            logger::log_error(ret, "Partition error:");
                            response["status"] = "error";
                            response["message"] = "Partitioning failed";
                        } else {
                            std::cout << "DEBUG: Building indexes..." << std::endl;
                            ret = hec::buildIndex({pointDatasetID, secondPointDatasetID}, hec::IT_TWO_LAYER);
                            if (ret != DBERR_OK) {
                                logger::log_error(ret, "Index build error:");
                                response["status"] = "error";
                                response["message"] = "Index building failed";
                            } else {
                                std::cout << "DEBUG: Creating spatial join query..." << std::endl;
                                hec::QueryType joinType = mapPredicateToJoinType(predicate);
                                predicateJoinQuery = new hec::PredicateJoinQuery(pointDatasetID, secondPointDatasetID, 0, joinType, hec::QR_COLLECT);
                                if (predicateJoinQuery == nullptr) {
                                    response["status"] = "error";
                                    response["message"] = "Failed to create spatial join query";
                                    std::cout << "ERROR: Spatial join query creation failed" << std::endl;
                                } else {
                                    data_prepared = true;
                                    response["status"] = "success";
                                    std::cout << "DEBUG: Spatial join query prepared successfully" << std::endl;
                                }
                            }
                        }
                    }
                }
                else {
                    response["status"] = "error";
                    response["message"] = "Unknown query type: " + current_query_type;
                }
            }
            else if (action == "execute") {
                if (!data_prepared) {
                    response["status"] = "error";
                    response["message"] = "Data not prepared";
                } 
                else if (current_query_type == "knnQuery") {
                    start = hec::time::getTime();
                    results = hec::query(batch, hec::Q_KNN);
                    total_time = hec::time::getTime() - start;
                    response["status"] = "success";
                    response["executionTime"] = total_time;
                    
                    std::string timestamp = std::to_string(std::time(nullptr));         
                    json results_json;
                    results_json["timestamp"] = timestamp;
                    results_json["executionTime"] = total_time;
                    results_json["queryType"] = current_query_type;
                    
                    json results_array = json::array();
                    for (const auto& [queryID, resultPtr] : results) {
                        if (resultPtr) {
                            json result_item;
                            result_item["queryID"] = resultPtr->getQueryID();
                            result_item["queryType"] = resultPtr->getQueryType();
                            result_item["resultType"] = resultPtr->getResultType();
                            result_item["resultCount"] = resultPtr->getResultCount();
                            result_item["neighbors"] = resultPtr->getResultList();
                            results_array.push_back(result_item);
                        }
                    }
                    
                    results_json["results"] = results_array;
                    results_json["totalResults"] = results.size();
                    response["resultsData"] = results_json;
                } 
                else if (current_query_type == "rangeQuery") {
                    start = hec::time::getTime();
                    results = hec::query(batch, hec::Q_RANGE);
                    total_time = hec::time::getTime() - start;
                    response["status"] = "success";
                    response["executionTime"] = total_time;
                    
                    std::string timestamp = std::to_string(std::time(nullptr));
                    json results_json;
                    results_json["timestamp"] = timestamp;
                    results_json["executionTime"] = total_time;
                    results_json["queryType"] = current_query_type;
                    
                    json results_array = json::array();
                    for (const auto& [queryID, resultPtr] : results) {
                        if (resultPtr) {
                            json result_item;
                            result_item["queryID"] = resultPtr->getQueryID();
                            result_item["queryType"] = resultPtr->getQueryType();
                            result_item["resultType"] = resultPtr->getResultType();
                            result_item["resultCount"] = resultPtr->getResultCount();
                            result_item["neighbors"] = resultPtr->getResultList();
                            results_array.push_back(result_item);
                        }
                    }
                    
                    results_json["results"] = results_array;
                    results_json["totalResults"] = results.size();
                    response["resultsData"] = results_json;
                }
                else if (current_query_type == "distanceJoins") {
                    if (distanceQuery == nullptr) {
                        response["status"] = "error";
                        response["message"] = "Distance query not initialized";
                    } else {
                        start = hec::time::getTime();
                        hec::QResultBase* result = hec::query(distanceQuery);
                        total_time = hec::time::getTime() - start;
                        
                        response["status"] = "success";
                        response["executionTime"] = total_time;
                        
                        // Build response JSON
                        std::string timestamp = std::to_string(std::time(nullptr));
                        json results_json;
                        results_json["timestamp"] = timestamp;
                        results_json["executionTime"] = total_time;
                        results_json["queryType"] = current_query_type;
                        
                        if (result) {
                            json result_item;
                            result_item["queryID"] = result->getQueryID();
                            result_item["queryType"] = mapping::queryTypeIntToStr(result->getQueryType());
                            result_item["resultType"] = result->getResultType();
                            result_item["resultCount"] = result->getResultCount();
                            result_item["joinResults"] = result->getResultList();
                            
                            results_json["results"] = json::array({ result_item });
                            results_json["totalResults"] = 1;
                        } else {
                            results_json["results"] = json::array();
                            results_json["totalResults"] = 0;
                        }
                        
                        response["resultsData"] = results_json;
                    }
                }
                else if (current_query_type == "spatialJoins") {
                    if (predicateJoinQuery == nullptr) {
                        response["status"] = "error";
                        response["message"] = "Spatial join query not initialized";
                    } else {
                        start = hec::time::getTime();
                        hec::QResultBase* result = hec::query(predicateJoinQuery);
                        total_time = hec::time::getTime() - start;
                        
                        response["status"] = "success";
                        response["executionTime"] = total_time;
                        
                        // Build response using the existing mapping::queryTypeIntToStr function
                        std::string timestamp = std::to_string(std::time(nullptr));
                        json results_json;
                        results_json["timestamp"] = timestamp;
                        results_json["executionTime"] = total_time;
                        results_json["queryType"] = current_query_type;
                        results_json["predicate"] = mapping::queryTypeIntToStr(predicateJoinQuery->getQueryType());
                        
                        if (result) {
                            json result_item;
                            result_item["queryID"] = result->getQueryID();
                            result_item["queryType"] = mapping::queryTypeIntToStr(result->getQueryType());
                            result_item["resultType"] = result->getResultType();
                            result_item["resultCount"] = result->getResultCount();
                            result_item["joinResults"] = result->getResultList();
                            
                            results_json["results"] = json::array({ result_item });
                            results_json["totalResults"] = 1;
                        } else {
                            results_json["results"] = json::array();
                            results_json["totalResults"] = 0;
                        }
                        
                        response["resultsData"] = results_json;
                    }
                } 
                else {
                    response["status"] = "error";
                    response["message"] = "Unknown query type: " + current_query_type;
                }
            }
            else if (action == "clear") {
                if (hec_initialized) {
                    for (auto& q : batch) {
                        auto it = results.find(q->getQueryID());
                        if (it != results.end()) {
                            
                        }
                        delete q;
                    }
                    batch.clear();
                    results.clear();
                    
                    // Unload both datasets if they were loaded
                    if (pointDatasetID >= 0) {
                        hec::unloadDataset(pointDatasetID);
                        pointDatasetID = -1;
                    }
                    if (secondPointDatasetID >= 0) {
                        hec::unloadDataset(secondPointDatasetID);
                        secondPointDatasetID = -1;
                    }
                    
                    // Clean up queries if they exist
                    if (distanceQuery != nullptr) {
                        delete distanceQuery;
                        distanceQuery = nullptr;
                    }
                    if (predicateJoinQuery != nullptr) {
                        delete predicateJoinQuery;
                        predicateJoinQuery = nullptr;
                    }
                    
                    current_query_type = "";
                    data_prepared = false;
                }
                response["status"] = "clear";
            }
            else if (action == "terminate") {
                if (hec_initialized) {
                    for (auto& q : batch) {
                        auto it = results.find(q->getQueryID());
                        if (it != results.end()) {
                            
                        }
                        delete q;
                    }
                    batch.clear();
                    results.clear();
                    
                    // Unload datasets if they were loaded
                    if (pointDatasetID >= 0) {
                        hec::unloadDataset(pointDatasetID);
                        pointDatasetID = -1;
                    }
                    if (secondPointDatasetID >= 0) {
                        hec::unloadDataset(secondPointDatasetID);
                        secondPointDatasetID = -1;
                    }
                    
                    // Clean up queries if they exist
                    if (distanceQuery != nullptr) {
                        delete distanceQuery;
                        distanceQuery = nullptr;
                    }
                    if (predicateJoinQuery != nullptr) {
                        delete predicateJoinQuery;
                        predicateJoinQuery = nullptr;
                    }
                    
                    data_prepared = false;
                    current_query_type = "";
                    hec::finalize();
                    hec_initialized = false;
                }
                response["status"] = "terminated";
                std::cout << "C++: " << response.dump() << std::endl;
                std::cout.flush();
                break;
            } else {
                response["status"] = "error";
                response["message"] = "Unknown action: " + action;
            }
            
            std::cout << "C++: " << response.dump() << std::endl;
            std::cout.flush();

        } catch (const std::exception& e) {
            json err;
            err["status"] = "error";
            err["message"] = e.what();
            std::cout << "C++: " << err.dump() << std::endl;
            std::cout.flush();
        }
    }
    return 0;
}