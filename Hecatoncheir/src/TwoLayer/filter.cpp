        ...
        static DB_STATUS evaluate(hec::RangeQuery *rangeQuery, hec::QueryResult &queryResult) {
            ...
            #pragma omp parallel reduction(query_output_reduction:queryResult)
            {
                ...
                if (partitionMinX == partitionMaxX && partitionMinY == partitionMaxY) {
                    int partitionID = g_config.partitioningMethod->getPartitionID(partitionMinX, partitionMinY, g_config.partitioningMethod->getGlobalPPD());
                    PartitionBase* partition = dataset->index->getPartition(partitionID);
                    ...
                    if (partition != nullptr) {
                        // valid partition
                        /** @todo range corners ABCD */
                    }
                    //intermidiate tiles in the row
                    for (int i=partitionMinX+1; i<partitionMaxX; i++) {
                        /** @todo range corners AB */
                    ...
                    for (int j=partitionMinY+1; j<partitionMaxY; j++) {
                        ...
                        //end tile in the row   
                        /** @todo range border A vertically */
                    ...
                    //last row 
                    //first tile in the row
                    /** @todo range corners AC */

                    //intermidiate tiles in the row
                    for (int i=partitionMinX+1; i<partitionMaxX; i++) {
                        /** @todo range border A horizontally */
                    }

                    //last tile in the row
                    /** @todo range corners A */
