...
if [ "$#" -eq 0 ]; then
  # default case (local)
  echo "building using default build type: $systype, build mode: $buildtype"
elif [ "$#" -eq 1 ]; then
  # specified type
  systype=$1
  ...
  echo "building using specified build type: $systype, default build mode: $buildtype"
elif [ "$#" -eq 2 ]; then
  systype=$1
  buildtype=$2
  if [[ "$systype" != "local" && "$systype" != "cluster" ]]; then
  ...
  echo "building using specified build type: $systype, build mode: $buildtype"
else
  # wrong
